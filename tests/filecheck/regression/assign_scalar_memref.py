...
# Exercises: exo.alloc (scalar → memref[1]), exo.assign (scalar memref),
#            exo.read (scalar memref), reduce (scalar += via read + add + assign)
...
# CHECK:        %3 = "llvm.load"({{.*}}) <{ordering = 0 : i64}> : (!llvm.ptr) -> f32
