...
class JitFunc:
    ...
    __slots__ = ("_arg_names", "_engine", "_fn", "_marshallers")

    def __init__(self, address: int, engine: object, arg_kinds: bytes, arg_names: Sequence[str]):
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        if kwargs:
            args = tuple(kwargs[name] for name in self._arg_names)
...
def _jit_compile(proc: Procedure, raw: bool = False) -> Callable[..., None] | JitFunc:
    ...
    raw_fn = JitFunc(engine.get_function_address(proc.name()), engine, arg_kinds, [arg.name.name() for arg in proc._loopir_proc.args])
