"""Exo compatibility patches for tensors containing ``size`` or ``index``.

Exo accepts those types as scalar procedure arguments, but does not expose them
as tensor element types. Its bounds checker also assumes that reads of
indexable values are scalar. ExoJIT needs both forms for integer index buffers.
"""
...
_ORIGINAL_LIFT_EXPR = _boundscheck.lift_expr
_LIFTED_INDEX_SYMS: dict[tuple[object, ...], Sym] = {}


def _patched_lift_expr(e):
    ...
    if not (isinstance(e, LoopIR.Read) and e.idx and e.type.is_indexable()):
        return _ORIGINAL_LIFT_EXPR(e)
    key = expr_key(e)
    sym = _LIFTED_INDEX_SYMS.get(key)
    if sym is None:
        sym = Sym(f"lifted_index_{len(_LIFTED_INDEX_SYMS)}")
        _LIFTED_INDEX_SYMS[key] = sym
    return _boundscheck.E.Var(sym, e.type, e.srcinfo)


def apply_exo_patches() -> None:
    """Enable ``size[...]`` and ``index[...]`` arguments in Exo procedures."""
    if _boundscheck.lift_expr is _patched_lift_expr:
        return

    assert _boundscheck.lift_expr is _ORIGINAL_LIFT_EXPR, "Exo's bounds checker was already patched"
    _pyparser._prim_types["size"] = UAST.Size()
    _pyparser._prim_types["index"] = UAST.Index()
    _boundscheck.lift_expr = _patched_lift_expr
